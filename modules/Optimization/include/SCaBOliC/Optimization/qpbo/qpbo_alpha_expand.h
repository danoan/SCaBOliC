...
auto energy(const Unary& U,
  ...
  for (typename Graph::Index i = 0; i < G.outerSize(); ++i) {
    ...
    for (typename Graph::InnerIterator it(G, i); it; ++it) {
      e += (*Ps[it.value() - 1])(l[it.col()], l[i],it.col(),i); //TODO::I've modified here!!!!
    }
  }
    ...
    for (Index alpha = 0; alpha < num_labels; ++alpha) {
      ...
      for (Index j = 0; j < num_nodes; ++j) {
        ...
        for (typename Graph::InnerIterator it(
          ...
          if (ei[k] < 0) {
            E0 += P(l[k], l[j],k,j);  //TODO:: I've modified here!!!
            E1 += P(l[k], alpha,k,j); //TODO:: I've modified here!!!
          } else {
            ...
            qpbo->AddPairwiseTerm(
              ...
              P(l[k], l[j],k,j), P(alpha, l[j],k,j), P(l[k], alpha,k,j), P(alpha, alpha,k,j)); //TODO:: I've modified here!!!
          }
        }
